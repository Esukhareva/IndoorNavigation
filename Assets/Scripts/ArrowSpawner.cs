...
public class ArrowSpawner : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.name.Equals("NavTrigger(Clone)") && line.positionCount > 0)
        {
            ...
            Vector3 pos = arcoreDeviceCam.transform.position + arcoreDeviceCam.transform.forward * 2 + arcoreDeviceCam.transform.up * -1.5f;
            // rotate arrow a bit
        ...
        if (other.tag.Equals("destination") && line.positionCount > 0 && GameObject.Find("Path").GetComponent<PathDrawer>().target.name.Equals(other.name))
        {
            Destroy(GameObject.Find("NavTrigger(Clone)"));
            Destroy(GameObject.Find("Anchor"));
            GameObject.Find("Path").GetComponent<PathDrawer>().target = null;
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.tag.Equals("destination") && GameObject.Find("Path").GetComponent<PathDrawer>().target.name.Equals(other.name))
        {
            Destroy(GameObject.Find("Anchor"));
        }
